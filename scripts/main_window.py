...
+ 键控音频播放现不受焦点窗口影响
+ 听写：分句时为标点符号追加语音提示
+ 配置文件(settings.ini)结构重构
+ 添加清除缓存功能
+ 欢迎页面添加在线服务可用性检测
+ 设置-AI设置大改，支持用户自定义模型id名称地址等（暂不支持anthropic协议）
+ 由于兼容性问题，TTS模型现强制使用edgetts而无法自定义。
+ Gitee内容源初步支持
+ SEI现已被废弃，改为用户手动复制教材详情页链接+自动解析下载识图导入。
+ 插件选项卡现在默认可见。
+ 补齐了新手引导-选项卡排序与可见性部分中缺失的流媒体按钮
+ 大改新手引导以适应新特性，并添加了字体设置、热键设置等新页面
