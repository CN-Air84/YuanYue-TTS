...
class PluginManager:
    ...
    def discover_plugins(self) -> List[PluginMetadata]:
        ...
        for plugin_path in self.plugin_directory.iterdir():
            ...
            try:
                ...
                if metadata:
                    metadata.directory_name = plugin_path.name
                    discovered_plugins.append(metadata)
                    logger.info(f"Discovered plugin: {metadata.name} v{metadata.version} (dir={plugin_path.name})")
    ...
    def disable_plugin(self, plugin_name: str) -> bool:
        ...
        try:
            ...
            self.plugin_api.cleanup_theme_subscriptions(plugin_name)
            ...
            def run_hook():
                ...
                try:
                    if hook_name == 'on_enable':
                        hook_func(api=self.plugin_api, plugin_name=plugin.get_plugin_name())
                    else:
                        hook_func()
                except Exception as e:
                    hook_exception = e
