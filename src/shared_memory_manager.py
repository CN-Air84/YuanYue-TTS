...
class SharedMemoryManager(QObject):
    ...
    def broadcast_theme_commit(self, event_data):
        """只在权威主题提交成功后广播固定版本事件。"""
        debug_logger.output(
            "shared_memory_manager.py",
            LogLevel.INFO,
            f"正在广播主题提交 revision={event_data.get('revision', 'unknown')}",
            fold_code="SHARED_MSG",
        )
        try:
            self._ensure_initialized()
            message = {
            ...
            debug_logger.output(
                "shared_memory_manager.py", LogLevel.ERROR,
                f"广播主题提交失败: {str(e)}", fold_code="SHARED_MSG",
            )
    ...
    def _process_message(self, message):
        ...
        try:
            ...
            elif msg_type in ('theme_change', 'theme_commit'):
