...
class PluginAPI:
    ...
    def register_tab(
        ...
        order: int = 999,
        *,
        transparent_theme_root: bool = False,
    ) -> bool:
        """
        ...
        Args:
            ...
            transparent_theme_root: 显式加入主题透明根层；默认保持旧插件实体样式
        ...
        try:
            ...
            def class_getter():
                """Return a lazy factory so setup runs on the real page instance."""

                def page_factory(parent=None):
                    try:
                        page_instance = widget_class(parent)
                        self.configure_plugin_theme_root(
                            page_instance,
                            transparent=transparent_theme_root,
                        )
                        ...
                        if hasattr(page_instance, 'on_page_created'):
                            try:
                                ...
                                logger.info(
                                    f"Plugin page '{tab_name}' created for plugin '{plugin_name}'"
                                )
                            except Exception as lifecycle_error:
                                logger.error(
                                    f"Error in on_page_created for plugin '{plugin_name}', "
                                    f"tab '{tab_name}': {lifecycle_error}",
                                    exc_info=True,
                                )
                        ...
                        self._start_memory_monitoring(
                            plugin_name, tab_name, page_instance
                        )
                        return page_instance
                    except Exception as error:
                        logger.error(
                            f"Error loading plugin page: {error}", exc_info=True
                        )
                        self.plugin_manager.plugin_logger.error(
                            f"Failed to load plugin page '{tab_name}': {error}",
                            plugin_name,
                        )
                        error_page_class = self._create_error_page_class(
                            plugin_name, tab_name, error
                        )
                        return error_page_class(parent)

                page_factory.__name__ = getattr(
                    widget_class, "__name__", "PluginPageFactory"
                )
                return page_factory
    ...
    # 主题系统契约
    def configure_plugin_theme_root(
        self,
        widget,
        *,
        transparent: bool = False,
    ):
        """Configure a plugin page root without changing legacy defaults.

        Old plugins stay opaque and outside the scoped installer.  A plugin
        must explicitly request a transparent root before its controls can
        participate in layered theme effects.
        """
        if widget is None or not hasattr(widget, "setProperty"):
            raise TypeError("widget must be a QWidget-compatible object")
        if transparent:
            configure_transparent_root(widget)
            widget.setProperty("legacyPlugin", False)
            widget.setProperty("themeEffectsRootDisabled", False)
            widget.setProperty("themeEffectDisabled", False)
            widget.setProperty("themeGlowDisabled", False)
        else:
            widget.setProperty("legacyPlugin", True)
    ...
    def configure_theme_widget(
        ...
        *,
        ...
        """Apply the public category and per-widget opt-out properties."""
        return configure_effect_widget(
            widget,
            category=category,
            effect_disabled=effect_disabled,
            glow_disabled=glow_disabled,
        )

    def _get_theme_service(self):
        main_window = getattr(self.plugin_manager, "main_window", None)
        return getattr(main_window, "theme_service", None)

    def subscribe_theme_commits(
        self,
        plugin_name: str,
        callback: Callable,
        *,
        ...
        """Subscribe to immutable committed theme snapshots.

        Preview drafts are intentionally excluded.  The returned callable is
        idempotent and releases the subscription.
        """
        if plugin_name not in self.plugin_manager.plugins:
            logger.error(f"Plugin not found: {plugin_name}")
            return None
        ...
        service = self._get_theme_service()
        if service is None or not hasattr(service, "subscribe_commit"):
            logger.warning("ThemeService is not available for plugin subscription")
            return None

        def deliver(event_or_theme):
            theme = getattr(event_or_theme, "theme", event_or_theme)
            try:
                callback(theme)
            except Exception as error:
                logger.error(
                    f"Theme callback failed for plugin '{plugin_name}': {error}",
                    exc_info=True,
                )

        service_unsubscribe = service.subscribe_commit(deliver)
        subscriptions = self._theme_subscriptions.setdefault(plugin_name, set())
        cancelled = False
        ...
        def cancel():
            ...
            cancelled = True
            service_unsubscribe()
            subscriptions.discard(cancel)
            if not subscriptions:
                self._theme_subscriptions.pop(plugin_name, None)

        subscriptions.add(cancel)
        if emit_current:
            deliver(service.active_theme)
        return cancel

    def subscribe_theme_commit(
        self,
        plugin_name: str,
        callback: Callable,
        *,
        ...
        """Singular-name compatibility alias for subscribe_theme_commits."""
        return self.subscribe_theme_commits(
            plugin_name,
            callback,
            emit_current=emit_current,
        )

    def cleanup_theme_subscriptions(self, plugin_name: str) -> int:
        """Release all theme callbacks owned by a plugin."""
        subscriptions = tuple(self._theme_subscriptions.get(plugin_name, ()))
