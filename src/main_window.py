...
+ 主题系统重写：支持控件与卡片模糊效果、文字发光，支持将图片设为背景，下拉框翻新，七套新主题
+ （实验功能）添加从pdf导出音频功能，支持将整本书一次性转化为可听的音频。
+ 听写功能微调：不再弹出“正在生成中”弹窗提示
+ 欢迎：可变字体slogan自动扫描可用字体
+ 欢迎：删除快捷导航卡片，公告栏增高
+ 流媒体：歌单列表UI修复
+ 流媒体：修复UI缩放问题
+ 流媒体：修复自动连播不生效问题
+ 流媒体：歌词显示逻辑优化
+ 流媒体：添加歌单加载动画，修复加载时前端进程阻塞问题
+ 流媒体：优化歌词滚动逻辑 
+ 流媒体：添加歌曲简介悬浮窗
+ 多处UI微调优化
+ 还有什么改动来着……忘掉了……
...
from PyQt5.QtGui import QFont, QPainter, QColor, QPainterPath, QPalette
...
from theme_page_adapter import configure_transparent_container
...
class AsyncInitializer:
    ...
    def _step_create_single_page(self, page_index, is_initial=False):
        ...
        try:
            ...
            if getattr(pw, "theme_runtime", None) is not None:
                pw.theme_runtime.attach_page(page_widget)
...
class TabManager:
    ...
    def switch_to_tab(self, index):
        ...
        if index == self.current_tab_index:
            self._sync_tab_buttons(self.current_tab_index)
            return True
        ...
        switch_generation = self._tab_switch_generation
        ...
        try:
            animation_started = bool(self.parent_window._start_tab_fade_animation(
                from_index,
                index,
                on_finished=(
                    lambda: self._on_tab_transition_finished(index, switch_generation)
                    if page_ready else None
                ),
            ))
        ...
        # 动画初始化失败时仍然要完成切换后的轻量处理；正常路径由淡入动画
        # 完成回调触发，避免在动画帧之间同步重建音色控件。
        if page_ready and not animation_started:
            QTimer.singleShot(
                0,
                lambda: self._on_tab_transition_finished(index, switch_generation),
            )
    ...
    def _sync_tab_buttons(self, selected_index):
        for button_index, button in enumerate(self.tab_buttons):
            button.setChecked(button_index == selected_index)

    def _can_leave_current_tab(self):
        if not (0 <= self.current_tab_index < len(self.tab_configs)):
            return True
        if self.tab_configs[self.current_tab_index].name != "personalization":
            return True
        guard = getattr(self.parent_window, "_guard_unsaved_theme_draft", None)
        return True if guard is None else bool(guard())

    def _on_tab_transition_finished(self, index, switch_generation):
        """在转场完成后执行选项卡后处理，忽略已经过期的切换请求。"""
        if switch_generation != self._tab_switch_generation:
            return
        if index != self.current_tab_index:
            return
    ...
    def _on_tab_switched(self, index):
        ...
        if index < len(self.tab_configs):
            ...
            # 页面异步就绪可能早于当前转场结束（例如首次点击尚未加载的
            # 页面）。沿用同一完成回调，避免在动画帧中同步刷新控件。
            fade_timer = getattr(self.parent_window, "tab_fade_animation", None)
            if (
                index == self.current_tab_index
                and fade_timer is not None
                and fade_timer.isActive()
            ):
                switch_generation = self._tab_switch_generation
                self.parent_window._tab_fade_finished_callback = (
                    lambda: self._on_tab_transition_finished(index, switch_generation)
                )
                return
            ...
            if tab_config.name == 'dictation':
                dictation_page = self.parent_window.ready_gate.get("page_dictation")
                if dictation_page and hasattr(dictation_page, 'reload_current_voice_list'):
                    dictation_page.reload_current_voice_list()
...
class MainWindow(QWidget):
    ...
    def __init__(self, theme_runtime=None):
        ...
        self._init_ui()
        if self.theme_runtime is not None:
            self.theme_runtime.attach(self)
        # 不要在这里 show()，窗口在不可见位置
    ...
    def _init_core_components(self):
        ...
        try:
            ...
            self.settings_manager = (
                self.theme_runtime.settings_manager
                if self.theme_runtime is not None
    ...
    def _init_ui_variables(self):
        ...
        self._tab_fade_finished_callback = None
    ...
    def _setup_window_properties(self):
        ...
        self._apply_window_background_color(background_color)
    ...
    def _apply_window_background_color(self, background_color):
        """Set the root fallback color without cascading QSS into pages."""
    ...
    def _async_init_plugin_system(self):
        ...
        try:
            ...
            for plugin_metadata in discovered_plugins:
                try:
                    plugin_path = plugin_metadata.directory_name or plugin_metadata.name
        ...
        try:
            ...
            if page_name in ['custom', 'custom_page']:
                ...
                if bg_color:
                    ...
                    self._apply_window_background_color(bg_color)
    ...
    def _apply_deferred_font_update(self):
        ...
        self.font_manager.update_all_fonts()
        if self.theme_runtime is not None:
            self.theme_runtime.notify_font_context_changed()
    ...
    def resizeEvent(self, event):
        ...
        if self.theme_runtime is not None:
            self.theme_runtime.resize(width, height)
    ...
    def _guard_unsaved_theme_draft(self):
        """Ask the personalization page to resolve its draft before leaving."""
        ready_gate = getattr(self, "ready_gate", None)
        if ready_gate is None or not ready_gate.is_ready("page_personalization"):
            return True
        page = ready_gate.get("page_personalization")
        request_decision = getattr(page, "request_theme_navigation_decision", None)
        if request_decision is None:
            return True
        try:
            return bool(request_decision())
        except Exception as exc:
            debug_logger.output(
                "main_window.py",
                LogLevel.ERROR,
                f"处理未保存主题草稿时出错: {str(exc)}",
                fold_code="MAIN_EXIT",
            )
            return False
    ...
    def closeEvent(self, event):
        """处理窗口关闭事件"""
        if not self._guard_unsaved_theme_draft():
            event.ignore()
            debug_logger.output(
                "main_window.py",
                LogLevel.INFO,
    ...
    def _stop_tab_fade_animations(self):
        """停止并解除上一轮淡入/淡出动画，避免旧信号串入新转场。"""
        for attr_name, slot in (
            ("fade_out_animation", self._on_fade_out_finished),
            ("fade_in_animation", self._on_fade_in_finished),
        ):
            animation = getattr(self, attr_name, None)
            if animation is None:
                continue
            try:
                animation.finished.disconnect(slot)
    ...
    def _start_tab_fade_animation(self, from_index, to_index, on_finished=None):
        """开始选项卡淡入淡出动画"""
        ...
        self._stop_tab_fade_animations()

        # 新的切换请求取代旧请求，旧回调不能在后续切换中继续执行。
        self._tab_fade_finished_callback = on_finished
    ...
    def _on_fade_out_finished(self):
        ...
        # 开始淡入动画。先连接信号，避免极短动画（例如时长为 0）
        # 在连接前就发出 finished，导致后处理永远不执行。
        self.fade_in_animation.start()
    
    def _on_fade_in_finished(self):
        """淡入动画完成后的清理工作"""
        debug_logger.output("main_window.py", LogLevel.DEBUG, "淡入阶段结束，清理动画临时效果", fold_code="MAIN_ANIM")
        finished_callback = self._tab_fade_finished_callback
        self._tab_fade_finished_callback = None
        # 移除透明度效果
        try:
        ...
        # 让最终一帧先完成绘制，再执行可能包含文件读取和控件重建的后处理。
...
class SingleInstanceChecker:
    ...
    def is_already_running(cls):
        ...
        if getattr(sys, 'frozen', False):
            ...
            # 开发环境
            lock_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), '.lock')
...
def main():
    ...
    # 主题组合根必须在 QApplication 之后、MainWindow 首次构造和稳定绘制之前
    # 完成权威状态加载与旧配色迁移。
    theme_runtime = None
    try:
        from shared_memory_manager import get_shared_memory_manager
        from theme_runtime import build_theme_runtime
        ...
        debug_logger.output(
            "main_window.py", LogLevel.ERROR,
            f"主题运行时初始化失败，将使用旧实体样式: {str(e)}",
            fold_code="THEME_RUNTIME",
        )
    ...
    def _create_main_window():
        ...
        window = MainWindow(theme_runtime=theme_runtime)
        ...
        if theme_runtime is not None:
            loading_anim.animation_finished.connect(
                theme_runtime.notify_startup_blur_removed
            )
