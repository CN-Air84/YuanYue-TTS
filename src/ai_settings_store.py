...
class CustomAIModelStore:
    ...
    def apply_edgetts_tts_selection(self, broadcast: bool = True) -> None:
        """选中 EdgeTTS 并写入 TTS 路由所需的 legacy 配置。"""
        ...
        if broadcast:
            self._broadcast_tts_change(EDGE_TTS_PROVIDER, EDGE_TTS_MODEL)

    def select_cloud_tts(self, model_id: str, broadcast: bool = True) -> None:
        """选中在线API TTS模型。"""
        m = self.get_model_by_id(model_id)
        if not m or m.get("scene") != "tts":
            return
        self.set_active_id("tts", model_id)
        provider = m.get("provider", "")
        model = m.get("model", "")
        self.settings.Custom.set_value(EDGE_TTS_LEGACY_PROVIDER_KEY, provider)
        self.settings.Custom.set_value(EDGE_TTS_LEGACY_MODEL_KEY, model)
        if broadcast:
            self._broadcast_tts_change(provider, model)

    def _broadcast_tts_change(self, provider: str, model: str) -> None:
        try:
