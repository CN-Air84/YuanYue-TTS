...
from tts_engine_interface import (
    TTSEngineError,
...
class TTSRouter:
    """
    TTS 路由器 — 根据用户配置将合成任务路由到正确的引擎。

    架构:
      ┌──────────────┐
      │ generation_  │  TTSGenerationConfig(text, voice, speed, ...)
      │ page_neo     │────────────────────────────────────────────────┐
      └──────────────┘                                                │
                                                                      ▼
      ┌───────────────────────────────────────────────────────────────┐
      │                        TTSRouter                              │
      │  get_selected_provider() → 读 settings.ini                    │
      │  返回 "EdgeTTS" | "ChatGLM" | ...    │
      │                                                                │
      │  generate_audio(config)                                       │
      │   ├─ provider.startswith("plugin:") → _generate_with_dynamic_engine()
      │   ├─ provider == "EdgeTTS"           → _generate_with_edgetts()
      │   ├─ provider in AI providers        → _generate_with_ai_provider()
      │   └─ 否则                            → 明确失败（可显式启用 Edge 回退）
      └───────────────────────────────────────────────────────────────┘
    """
    ...
    def __init__(self):
        ...
        self._edge_generator_lock = threading.Lock()
        ...
        self._dynamic_engine_lock = threading.Lock()
    ...
    def get_selected_provider(self) -> str:
        ...
        # Settings are saved as separate keys. During that short write window one
        # read can observe both values as empty; keep the last complete selection.
        if not provider and not model and self._cached_provider and self._cached_model:
            debug_logger.output("tts_router.py", LogLevel.WARNING,
                f"检测到瞬时空TTS配置，继续使用上次有效配置: "
                f"{self._cached_provider}/{self._cached_model}",
                fold_code="TTS_ROUTER")
    ...
    def generate_audio(self, config: TTSGenerationConfig) -> bool:
        """
        生成音频 — 根据用户配置路由到对应的 TTS 服务。

        路由表:
          EdgeTTS              → AudioGenerator  (微软免费在线 TTS)
          ChatGLM / Qwen / ... → AI Manager       (付费 API TTS)

        Args:
            config: TTSGenerationConfig — text, voice, speed, pitch, volume, save_path

        Returns:
            bool: 是否生成成功 (True = 已写入 save_path)
        """
        ...
        if provider and provider.startswith("plugin:"):
            ...
            message = f"TTS 提供商不可用: {provider or '未配置'}"
            if self._edge_fallback_enabled():
                debug_logger.output("tts_router.py", LogLevel.WARNING,
                    f"{message}，用户已启用 EdgeTTS 回退",
                    fold_code="TTS_ROUTER")
                return self._generate_with_edgetts(config)
            raise TTSEngineUnavailableError(message)

    def _edge_fallback_enabled(self) -> bool:
        if not self.settings_manager:
            return False
        try:
            value = self.settings_manager.Custom.get_value(
                "tts_edge_fallback_enabled", False
            )
        except Exception:
            return False
    ...
    def _generate_with_edgetts(self, config: TTSGenerationConfig) -> bool:
        ...
        if self._edge_generator is None:
            edge_generator_lock = getattr(self, "_edge_generator_lock", None)
            if edge_generator_lock is None:
                edge_generator_lock = self._edge_generator_lock = threading.Lock()
            with edge_generator_lock:
                if self._edge_generator is None:
        ...
        # When falling back from plugin/other provider, ensure a valid EdgeTTS voice
        voice = config.voice
        if voice and not voice.startswith(("zh-CN", "en-US", "en-GB", "ja-JP", "ko-KR", "fr-FR", "de-DE", "pt-BR", "it-IT", "es-ES", "ru-RU", "ar-EG", "ar-SA", "vi-VN", "th-TH", "id-ID", "ms-MY", "fil-PH", "ta-IN", "te-IN", "am-ET", "bn-BD", "gu-IN", "hi-IN", "kn-IN", "ml-IN", "mr-IN", "ne-NP", "si-LK", "ur-IN", "ca-ES", "cy-GB", "da-DK", "el-GR", "fi-FI", "he-IL", "hr-HR", "hu-HU", "lt-LT", "lv-LV", "nb-NO", "nl-NL", "pl-PL", "ro-RO", "sk-SK", "sl-SI", "sv-SE", "tr-TR", "uk-UA", "yue-CN", "wuu-CN", "nan-CN", "hak-CN", "cdo-CN", "hsn-CN", "czh-CN", "hsn-CN", "cjy-CN", "lzh-CN")):
            default_edge = "zh-CN-Yunjian"
            debug_logger.output("tts_router.py", LogLevel.WARNING,
                f"回退EdgeTTS使用无效音色 {voice}，替换为 {default_edge}",
                fold_code="TTS_ROUTER")
            voice = default_edge
    ...
    def _generate_with_ai_provider(self, provider: str, config: TTSGenerationConfig) -> bool:
        ...
        try:
            ...
            if response.success:
                ...
                if self._edge_fallback_enabled():
                    debug_logger.output("tts_router.py", LogLevel.WARNING,
                        "AI TTS生成尚未实现，用户已启用 EdgeTTS 回退",
                        fold_code="TTS_ROUTER")
                    return self._generate_with_edgetts(config)
                raise TTSEngineError(f"{provider} TTS 生成尚未实现")
            else:
            ...
            if self._edge_fallback_enabled():
                return self._generate_with_edgetts(config)
            raise TTSEngineError(f"{provider} TTS 生成失败: {e}") from e
    ...
    def _generate_with_dynamic_engine(self, engine_id: str, config: TTSGenerationConfig) -> bool:
        ...
        流程:
          1. 查找注册的引擎类 (register_dynamic_engine 注册的)
          2. 创建或复用引擎实例
          3. 调用 engine.synthesize(text, voice, speed, pitch, volume, save_path)
          4. synthesize() 内部通过 stdin/stdout 管道与 tts_worker 子进程通信
          5. 成功返回 True，失败则保留插件错误语义
        ...
        try:
            ...
            if engine_id not in self._dynamic_engines:
                ...
                raise TTSEngineUnavailableError(
                    f"插件引擎 {engine_id} 未注册；请检查插件是否已启用"
                )
            
            # Synthesis and prewarming must share the same lock-protected instance.
            engine = self.get_or_create_dynamic_engine(engine_id)
            
            # 调用引擎的 synthesize 方法
            debug_logger.output("tts_router.py", LogLevel.INFO,
                f"使用插件引擎生成音频: {engine_id}, voice={config.voice}, speed={config.speed}",
                fold_code="TTS_ROUTER")
            ...
            if audio_path:
                ...
                debug_logger.output("tts_router.py", LogLevel.ERROR,
                    "插件引擎未返回音频文件",
                    fold_code="TTS_ROUTER")
                raise TTSEngineError(f"插件引擎 {engine_id} 未返回音频文件")
            ...
            debug_logger.output("tts_router.py", LogLevel.ERROR,
                f"插件引擎生成出错: {e}",
                fold_code="TTS_ROUTER")
            raise TTSEngineError(f"插件引擎 {engine_id} 生成失败: {e}") from e
    ...
    def get_or_create_dynamic_engine(self, engine_id: str):
        """Return the shared instance used by synthesis and background prewarming."""
        with self._dynamic_engine_lock:
            if engine_id not in self._dynamic_engines:
                raise KeyError(f"Dynamic TTS engine is not registered: {engine_id}")
            if engine_id not in self._dynamic_engine_instances:
                self._dynamic_engine_instances[engine_id] = self._dynamic_engines[engine_id]()
            return self._dynamic_engine_instances[engine_id]
        ...
        try:
            ...
            if engine_id in self._dynamic_engine_instances:
                ...
                try:
                    ...
                    debug_logger.output("tts_router.py", LogLevel.WARNING,
                        f"Engine cleanup failed: {engine_id}: {e}",
                        fold_code="TTS_ROUTER")
