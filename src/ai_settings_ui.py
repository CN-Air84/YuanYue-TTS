...
from theme_page_adapter import set_transparent_scroll_content
...
class EdgeTtsCard(QFrame):
    ...
    def __init__(self, metrics: Dict[str, Any], parent=None):
        ...
        self.setToolTip("系统内置 EdgeTTS，基于 Microsoft Edge 浏览器 TTS 引擎")
        self._title = QLabel("EdgeTTS")
        self._prov = QLabel("Microsoft")
        self._badge = QLabel("系统内置 · 始终可用")
        self._build_chrome()
...
class TTSChoiceCard(QFrame):
    """可点击的 TTS 引擎卡片。"""
    clicked = pyqtSignal()

    def __init__(self, engine_id: str, engine_name: str, provider: str,
                 active: bool, status_text: str, metrics: Dict[str, Any],
                 model_info: Dict[str, Any] = None, parent=None):
        super().__init__(parent)
        self.engine_id = engine_id
        self._metrics = dict(metrics)
        self._active = active
        self.setCursor(Qt.PointingHandCursor)
        self._title = QLabel(engine_name)
        self._prov = QLabel(provider)
        ...
        if model_info:
            dev_info = model_info.get("device_info", {})
            if dev_info:
                vram = dev_info.get("vram_mb", 0)
                ram = dev_info.get("ram_mb", 0)
                detail = f"{dev_info.get('device', 'cpu').upper()} · "
                if vram:
                    detail += f"显存{vram}MB · "
                if ram:
                    detail += f"内存{ram}MB"
                dt = QLabel(detail.strip(" · "))
                dt.setStyleSheet(
                    f"color: #888; font-size: {self._metrics['card_sub_px']}px; "
                    "border: none; background: transparent;"
                )
                dt.setFont(_metrics_font(self._metrics, "card_sub_px"))
                lay.addWidget(dt)

    def _build_chrome(self):
        m = self._metrics
        if self._active:
            border, bg = "#00CC66", "#F0FFF4"
        else:
            border, bg = "#E0E0E0", "#FFFFFF"
        ...
        self._status.setStyleSheet(
            f"color: #448855; font-size: {m['card_tag_px']}px; border: none; background: transparent;"
        )
        self._status.setFont(_metrics_font(m, "card_tag_px"))
...
class AddModelWidget(QWidget):
    ...
    def _init_ui(self):
        ...
        set_transparent_scroll_content(scroll, body)
...
class SelectModelWidget(QWidget):
    ...
    def _init_ui(self):
        ...
        set_transparent_scroll_content(scroll, self._scroll_body)
        outer.addWidget(scroll)
    ...
    def _rebuild_scene_cards(self):
        for sk, grid in self._scene_grids.items():
            ...
            cols = 3
            if sk == "tts":
                self._rebuild_tts_cards(grid, cols)
                continue
    ...
    def _rebuild_tts_cards(self, grid, cols):
        """重建文->音场景卡片。"""
        online_row = 0
        online_hdr = QLabel("☁ 在线API")
        online_hdr.setStyleSheet(style_section(self._metrics))
        online_hdr.setFont(_metrics_font(self._metrics, "section_font_px"))
        grid.addWidget(online_hdr, online_row, 0, 1, cols)

        online_card_row = online_row + 1
        online_col = 0

        # EdgeTTS card (always available, free online API)
        is_edge_active = self.store.is_edgetts_active_tts()
        edge_card = TTSChoiceCard(
            engine_id="edge-tts",
            engine_name="EdgeTTS",
            provider="Microsoft",
            active=is_edge_active,
            status_text="系统内置 · 免费 · 需联网",
            metrics=self._metrics,
        )
        edge_card.clicked.connect(lambda: self._select_edge_tts())
        grid.addWidget(edge_card, online_card_row, online_col)
        online_col += 1

        # Cloud API models
        cloud_models = [
            m for m in self.store.models_for_scene("tts")
            if self._model_passes_filter(m)
        ]
        is_cloud_active = not is_edge_active
        active_id = self.store.get_active_id("tts")
        for m in cloud_models:
            card = ModelCard(
                m, m.get("id") == active_id and is_cloud_active, self._metrics,
            )
            card.clicked.connect(lambda mid: self._select_cloud_tts(mid))
            card.long_pressed.connect(self.edit_model_requested.emit)
            grid.addWidget(card, online_card_row, online_col)
            online_col += 1

        if not cloud_models and not is_edge_active:
            pass  # EdgeTTS fills the first slot

    def _select_edge_tts(self):
        self.store.apply_edgetts_tts_selection(broadcast=True)
        self.refresh()

    def _select_cloud_tts(self, model_id: str):
        self.store.select_cloud_tts(model_id, broadcast=True)
        self.refresh()
    ...
    def _rebuild_groups(self):
        ...
        for g in self.store.groups:
            ...
            for sk in SCENES:
                if sk == "tts":
                    if self.store.is_edgetts_active_tts():
                        parts.append(f"{SCENE_LABELS[sk]}: {EDGE_TTS_MODEL}")
                    else:
                        mid = g.get(sk, "")
...
class AiSettingsTab(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # 默认选择：仅在无已有选择时设为 EdgeTTS
        if not self.store.get_active_id("tts"):
