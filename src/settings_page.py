...
from PyQt5.QtWidgets import (
    ...
    QSpinBox, QDoubleSpinBox, QScrollArea, QCheckBox, QSlider,
...
class SmoothButton(QPushButton):
    ...
    def __init__(self, text, btn_type="normal", parent=None):
        ...
        if btn_type not in {"tab_level1", "tab_level2"}:
...
class DownloadSettingsGroup(QWidget):
    ...
    def _init_ui(self):
        ...
        configure_theme_card(container_frame)
...
class DictationSettingsGroup(QWidget):
    ...
    def _init_ui(self):
        ...
        configure_transparent_container(auto_play_group)
...
class TabSettingsGroup(QWidget):
    def __init__(self, parent=None):
        ...
        self._visibility_animation_serial = 0
    ...
    def _init_ui(self):
        ...
        configure_theme_card(left_container)
        ...
        configure_theme_card(self.drag_container_frame)
        ...
        configure_theme_card(right_container)
        ...
        self.visibility_buttons_layout = QVBoxLayout()
        self.visibility_buttons_layout.setContentsMargins(0, 0, 0, 0)
        self.visibility_buttons_layout.setSpacing(5)
        self.visibility_buttons_layout.setAlignment(Qt.AlignTop)
        
        visibility_layout.addWidget(vis_label_container)
        visibility_layout.addLayout(self.visibility_buttons_layout)
    ...
    def _refresh_visibility_buttons(self, animate=False):
        self._visibility_animation_serial += 1
        animation_serial = self._visibility_animation_serial

        if self._visibility_animation is not None:
        ...
        existing_buttons = {}
        old_positions = {}
        while self.visibility_buttons_layout.count():
            item = self.visibility_buttons_layout.takeAt(0)
            widget = item.widget()
            if isinstance(widget, VisibilityButton):
                existing_buttons[widget.tab_name] = widget
                if animate:
                    old_positions[widget.tab_name] = widget.pos()
            elif widget:
                widget.deleteLater()
        ...
        for name in self.tab_order:
            ...
            btn = existing_buttons.pop(name, None)
            if btn is None:
                btn = VisibilityButton(display_name, name, is_vis, self)
                btn.visibility_toggled.connect(self._on_visibility_changed)
            else:
                btn.setText(display_name)
                btn.is_visible = is_vis
                btn.update_style()
            self.visibility_buttons_layout.addWidget(btn)

        for btn in existing_buttons.values():
            btn.deleteLater()

        if animate and old_positions:
            QTimer.singleShot(
                0,
                lambda positions=old_positions, serial=animation_serial:
                    self._animate_visibility_button_reorder(positions, serial),
            )

    def _animate_visibility_button_reorder(self, old_positions, animation_serial):
        if animation_serial != self._visibility_animation_serial:
            return
        ...
        for index in range(self.visibility_buttons_layout.count()):
            btn = self.visibility_buttons_layout.itemAt(index).widget()
            if not isinstance(btn, VisibilityButton):
                continue

            start_pos = old_positions.get(btn.tab_name)
            end_pos = btn.pos()
            if start_pos is None or start_pos == end_pos:
                continue

            target_positions[btn] = end_pos
            btn.move(start_pos)

            animation = QPropertyAnimation(btn, b"pos", animation_group)
        ...
        self._visibility_animation = animation_group
        animation_group.finished.connect(
            lambda group=animation_group, targets=target_positions:
                self._finish_visibility_button_reorder(group, targets)
        )
        animation_group.start()

    def _finish_visibility_button_reorder(self, animation_group, target_positions):
        if self._visibility_animation is not animation_group:
            return

        for btn, target_pos in target_positions.items():
            btn.move(target_pos)
    ...
    def _on_order_changed(self):
        ...
        self._refresh_visibility_buttons(animate=True)
        self._update_initial_tab_combo()
        self._save_settings()
...
class SettingsPage(QWidget):
    ...
    def _init_ui(self):
        ...
        configure_theme_card(self.ai_settings_tab)
        
        self.download_scroll = QScrollArea()
        configure_transparent_container(self.download_scroll)
        ...
        set_transparent_scroll_content(self.download_scroll, self.download_group)
        
        self.import_scroll = QScrollArea()
        configure_transparent_container(self.import_scroll)
        ...
        set_transparent_scroll_content(self.import_scroll, self.dictation_group)
        
        self.tab_scroll = QScrollArea()
        configure_transparent_container(self.tab_scroll)
        ...
        set_transparent_scroll_content(self.tab_scroll, self.tab_settings_group)
