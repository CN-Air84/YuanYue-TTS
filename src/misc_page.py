...
from PyQt5.QtWidgets import (
    ...
    QTreeWidget, QTreeWidgetItem, QLineEdit, QCheckBox, QTabWidget, QTableWidget,
...
from theme_page_adapter import configure_transparent_root
...
class MiscPage(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.setStyleSheet(f"""
            MiscPage {{background-color: {self.background_color}; color: {self.text_color};}}
    ...
    def _setup_layout(self):
        ...
        positions = [
            (row, column)
            for row in range(BUTTON_GRID_ROWS)
        ...
        self._update_button_heights()

    def _update_button_heights(self):
        """让按钮高度随宽度缩放，同时保持紧凑。"""
        layout = self.layout()
        if layout is None:
            return
        ...
        horizontal_spacing = max(0, layout.horizontalSpacing())
        vertical_spacing = max(0, layout.verticalSpacing())
        available_width = max(
            0,
            self.width()
            - margins.left()
            - margins.right()
            - horizontal_spacing * (BUTTON_GRID_COLUMNS - 1),
        )
        available_height = max(
            ...
            - margins.top()
            - margins.bottom()
            - vertical_spacing * (BUTTON_GRID_ROWS - 1),
        )

        column_width = available_width / BUTTON_GRID_COLUMNS
        width_based_height = max(
            BUTTON_MIN_HEIGHT,
            int(round(column_width * BUTTON_HEIGHT_RATIO)),
        )
        row_capacity = max(
            BUTTON_MIN_HEIGHT,
            int(available_height / BUTTON_GRID_ROWS),
        )
        target_height = min(width_based_height, row_capacity)

        for button in self.buttons:
            if button.maximumHeight() != target_height:
