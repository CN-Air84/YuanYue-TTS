...
class TTSEngineError(Exception):
    """Base error raised by TTS engines with user-facing recovery metadata."""

    code = "TTS_ENGINE_ERROR"
    retryable = False
    action = "检查 TTS 引擎设置后重试"

    def __init__(self, message="TTS engine failed", *, code=None, retryable=None,
                 action=None):
...
class TTSEngineUnavailableError(TTSEngineError):
    """The selected engine or its external runtime is unavailable."""

    code = "TTS_ENGINE_UNAVAILABLE"
    retryable = True
    action = "检查所选 TTS 插件及其运行环境后重试"


class TTSEngineProtocolError(TTSEngineError):
    """The plugin and external runtime cannot negotiate a protocol version."""

    code = "TTS_ENGINE_PROTOCOL_ERROR"
    action = "升级 TTS 插件或主程序，使协议版本保持一致"


class TTSEngineTimeoutError(TTSEngineError):
    """The selected engine did not finish within its request deadline."""

    code = "TTS_ENGINE_TIMEOUT"
    retryable = True
    action = "检查运行时状态和设备负载后重试当前句子"
