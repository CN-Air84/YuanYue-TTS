...
from PyQt5.QtWidgets import (
    ...
    QPushButton, QCheckBox, QGroupBox, QTextEdit, QFileDialog,
    QMessageBox, QDateEdit, QComboBox
)
from PyQt5.QtCore import QProcess, QDate
from PyQt5.QtGui import QIntValidator
from debug_logger import debug_logger, LogLevel

EXCLUDED_MODULES = (
    "tkinter", "tkinter.filedialog", "tkinter.messagebox",
    "matplotlib", "matplotlib.pyplot", "IPython", "ipython", "jupyter",
    "notebook", "pytest", "_pytest", "unittest", "doctest", "sphinx",
    "docutils", "pygments", "pip", "setuptools", "distutils",
    "pkg_resources", "mypy", "mypy.version", "mypy.util",
    "mypy.typevars", "mypy.types", "mypy.server", "mypy.semanal",
    "mypy.plugins", "mypy.plugin", "mypy.options", "mypy.nodes",
    "hypothesis", "trio", "trio.testing", "trio.socket", "trio.lowlevel",
    "trio.to_thread", "trio.from_thread", "curio", "rich", "rich.pretty",
    "jinja2", "outcome", "uvloop", "pyodide", "pyodide.ffi", "js",
    "bcrypt", "zstandard", "brotlicffi", "compression", "socks",
    "chardet", "OpenSSL", "OpenSSL.crypto", "OpenSSL.SSL",
    "cryptography.x509.UnsupportedExtension", "html5lib",
    "html5lib.treebuilders", "BeautifulSoup", "bs4", "lxml_html_clean",
    "cssselect", "cython", "cython.cimports", "email_validator", "toml",
    "tomli", "dotenv", "eval_type_backport", "annotationlib",
    "trove_classifiers", "simplejson", "dummy_threading", "termios", "tty",
    "readline", "rlcompleto", "org", "org.python", "java", "java.lang",
    "vms_lib", "_scproxy", "usercustomize", "sitecustomize", "_typeshed",
    "websockets", "websockets.exceptions", "OpenGL",
)
...
class PackagerGUI(QWidget):
    ...
    def initUI(self):
        ...
        self.setWindowTitle('源悦 TTS 构建工具')
        self.setGeometry(100, 100, 720, 760)
        ...
        self.filename_input = QLineEdit(
            str(Path(get_app_base_path()) / "src" / "main_window.py")
        )
        ...
        self.icon_input = QLineEdit(
            str(Path(get_app_base_path()) / "assets" / "1.ico")
        )
        ...
        # 是否添加settings.ini
        self.add_settings_checkbox = QCheckBox('添加config/settings.ini')
        ...
        # Win7/8 兼容模式选项
        self.win7_compat_checkbox = QCheckBox('Win7/8 兼容模式(包含 UCRT 运行时)')
    ...
    def start_packaging(self):
        ...
        if any(not part for part in version_parts[:3]):
            QMessageBox.warning(self, "警告", "请填写完整的前三段版本号")
            return
        ...
        try:
            ...
            if update_content.strip():
                update_file_path = Path(get_app_base_path()) / "docs" / "update_log.txt"
                try:
    ...
    def run_next_package(self):
        ...
        # 构建命令参数
        args = ['-m', 'PyInstaller', f'--name={version}']
        args.extend(['--clean', '--noconfirm'])
        args.extend(f"--exclude-module={name}" for name in EXCLUDED_MODULES)
        src_path = str(Path(get_app_base_path()) / "src")
        if os.path.isdir(src_path):
            args.extend(["--paths", src_path])
        
        # 添加文件选项
        if self.add_settings_checkbox.isChecked():
            # 打包时附带 config/settings.ini
            settings_file = Path(get_app_base_path()) / "config" / "settings.ini"
            if settings_file.is_file():
                separator = ';' if sys.platform.startswith("win") else ':'
                args.append(f'--add-data={settings_file}{separator}.')
        ...
        if self.win7_compat_checkbox.isChecked():
            ...
            # 使用私有程序集，包含 UCRT 等运行时 DLL 到 exe 目录(Win7 可能缺少这些 DLL)
            args.append('--win-private-assemblies')
            # 添加 ucrt 的 hidden-import(Win7 需要 Universal C Runtime)
        ...
        if icon_path:
            ...
            # 使用默认图标
            args.append('--icon=' + os.path.join(get_app_base_path(), "assets", "1.ico"))
    ...
    def process_finished(self):
        ...
        debug_logger.output("packager_gui.py", LogLevel.INFO,
            f"PyInstaller 进程结束，退出代码: {exit_code}", fold_code="PKG_MAIN")
        ...
        debug_logger.output("packager_gui.py", LogLevel.INFO,
            "所有打包任务已执行完毕，开始后期处理", fold_code="PKG_CLEAN")
        self.pack_btn.setEnabled(True)
        self.pack_btn.setText('开始打包')

        try:
            self._restore_core_source()
        except Exception as e:
            self.log_output.append(f"打包完成后处理出错: {str(e)}")
            debug_logger.output("packager_gui.py", LogLevel.ERROR,
                f"后期处理过程出错: {str(e)}", fold_code="PKG_CLEAN")

        if exit_code == 0:
            self.log_output.append("打包完成!")
            QMessageBox.information(self, "完成", "打包已完成！")
        else:
            QMessageBox.critical(self, "打包失败", f"PyInstaller 退出代码: {exit_code}")

    def _restore_core_source(self):
        version = self.current_package_version
        original_path = Path(self.filename_input.text())
        original_dir = original_path.parent
        original_name = original_path.stem
        original_ext = original_path.suffix

        for spec_path in (
            original_dir / f"{version}.spec",
            original_dir / f"{original_name}_package.spec",
        ):
            if spec_path.exists():
                spec_path.unlink()
                self.log_output.append(f"已删除 {spec_path.name} 文件")
        ...
        for py_file in original_dir.glob("*.py"):
            try:
                ...
                self.log_output.append(f"备份文件 {py_file.name} 失败: {exc}")
